...
namespace ForkBot
{
	...
	public static int[] GetItemIDList(bool shoppable = false, bool includeBM = true)
        ...
        public static DateTime GetPropertyAsDateTime(string property)
        {
            using (var con = new SQLiteConnection(Constants.Values.DB_CONNECTION_STRING))
            {
                con.Open();
        ...
        public static void AddToProperty(string property, int amount)
        {
            var curr = Convert.ToInt32(GetProperty(property));
